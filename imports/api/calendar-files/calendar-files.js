...
export const CalendarFiles = new FilesCollection({
  ...
  storagePath(file) {
    ...
    if (file.meta && file.meta.calendarId)
      path += '/' + file.meta.calendarId;
    return path;
  },
  namingFunction(file) {
  ...
  onAfterUpload(file) {
    ...
    if (file.meta && file.meta.type && file.meta.calendarId) {
      if (file.meta.type == 'background')
        Meteor.call('calendars.update.background', file.meta.calendarId, file._id);
    }
  },
  onAfterRemove(files) {
    ...
    files.forEach(function(file) {
      if (file.meta && file.meta.type && file.meta.calendarId) {
      if (file.meta.type == 'background')
        Meteor.call('calendars.update.background', file.meta.calendarId, undefined);
      }
    }, this);
  }
});